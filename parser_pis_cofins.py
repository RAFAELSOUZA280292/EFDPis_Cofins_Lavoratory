...
def parse_efd_piscofins(
    ...
    for line in lines:
        ...
        if reg == "A170" and current_a100:
            ...
            # A lógica é: se o valor principal for zero, usa o valor OUTRAS
            if _to_float(vl_pis) == 0.0 and _to_float(_get(current_a100, 19)) > 0.0:
                vl_bc_pis = _get(current_a100, 18)
                vl_pis = _get(current_a100, 19)
            
            if _to_float(vl_cof) == 0.0 and _to_float(_get(current_a100, 21)) > 0.0:
