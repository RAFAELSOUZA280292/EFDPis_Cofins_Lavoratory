...
def exibir_aba_apuracao_com_dados(dados_m):
    ...
    st.success(f"✅ {total_registros} registros M processados")
    ...
    with tabs[0]:
        ...
        with sub_tabs_pis[2]:
            ...
            if not df_rec_pis.empty:
                colunas_valor = ['VL_REC_BRT', 'VL_BC_CONT', 'VL_CONT_APUR', 'VL_CONT_PER']
    ...
    with tabs[2]:
        st.markdown("### 📚 Tabelas de Referência")
        ...
        with sub_tabs_idx[0]:
            ...
            st.dataframe(df_idx_cod_cont, use_container_width=True, hide_index=True)
        
        with sub_tabs_idx[1]:
            st.markdown("#### 📋 Índice NAT_REC")
            st.dataframe(df_idx_nat_rec, use_container_width=True, hide_index=True)
        
        with sub_tabs_idx[2]:
            st.markdown("#### 📋 Índice NAT_BC_CRED")
            st.dataframe(df_idx_nat_bc, use_container_width=True, hide_index=True)
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        df_idx_cod_cont.to_excel(writer, sheet_name='INDICE COD_CONT', index=False)
        df_idx_nat_rec.to_excel(writer, sheet_name='INDICE NAT_REC', index=False)
        df_idx_nat_bc.to_excel(writer, sheet_name='INDICE NAT_BC_CRED', index=False)
    
    excel_data = output.getvalue()
    
    st.download_button(
        label="📥 Baixar Excel Completo (11 abas)",
        data=excel_data,
        file_name="apuracao_pis_cofins_completo.xlsx",
